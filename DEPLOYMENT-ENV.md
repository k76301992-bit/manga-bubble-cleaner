...
| `INFERENCE_TIMEOUT_MS` | الخادم فقط | لا | مهلة طلب ترميم منطقة واحدة؛ الافتراضي `120000` ميلي ثانية. القيمة القديمة `20000` كانت تُجهض طلبات سليمة على وحدات CPU المقيدة. |
| `INFERENCE_STARTUP_TIMEOUT_SEC` | الخادم فقط | لا | أقصى انتظار لإقلاع خدمة Python قبل تشغيل Node؛ الافتراضي `180` ثانية. |
| `INFERENCE_RETRIES` | الخادم فقط | لا | عدد محاولات ترميم المنطقة الواحدة؛ الافتراضي `2`. تغطي إعادة المحاولة فجوة إعادة تشغيل خدمة Python. |
| `INFERENCE_MAX_CONCURRENCY` | الخادم فقط | لا | سقف طلبات الترميم المتزامنة عالميًا عبر كل المهام؛ الافتراضي `2`. يمنع تكدّس الطلبات على الخدمة الداخلية. |
| `INFERENCE_NUM_THREADS` | الخادم فقط | لا | عدد خيوط torch/OpenCV داخل الخدمة؛ الافتراضي `1`. اتركه `1` على الحاويات ذات حصة CPU محدودة. |
| `QWEN_REQUEST_TIMEOUT_MS` | الخادم فقط | لا | مهلة استدعاء Qwen للصفحة الكاملة (يُنفّذ مرة واحدة لكل صفحة)؛ الافتراضي `300000` ميلي ثانية. |
| `TEXT_DETECTOR_TIMEOUT_MS` | الخادم فقط | لا | مهلة كاشف النص ONNX المحلي؛ الافتراضي `180000` ميلي ثانية. |
| `TEXT_DETECTOR_ENABLED` | الخادم فقط | لا | الافتراضي `true`. اجعله `false` فقط إذا أردت تشغيل fallback المحلي عمدًا. |
| `TEXT_DETECTOR_STRICT` | الخادم فقط | لا | الافتراضي `false`; يسمح بالانتقال إلى Qwen أو fallback المحلي عند انقطاع ONNX. اجعله `true` للتشخيص أو لمنع أي طلب بلا كاشف محلي. |
