...
const outputDir = join(root, "full-page-local-only", new Date().toISOString().replace(/[:.]/g, "-"));
...
async function main() {
  ...
  for (const name of names) {
    ...
    const result = await cleanImageInMemory({ image, mimeType, quality: "balanced" });
    const seconds = Number(((performance.now() - started) / 1000).toFixed(3));
    const outputName = `${basename(name, extname(name))}-local-only.png`;
  ...
  await writeFile(join(outputDir, "summary.json"), JSON.stringify({ mode: "balanced-local-only", input: "complete original pages", pages }, null, 2));
