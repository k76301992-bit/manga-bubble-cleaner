...
> **الحالة الحالية:** الخادم جاهز للنشر التقني واختُبر أمر إنتاجه (`pnpm build` ثم `pnpm start`) ونقطة صحته `GET /api/health`. لكن فحصًا بصريًا جديدًا على مقطع مانهوا فعلي كشف بقايا غير مقبولة حول النص في فقاعة معيّنة. لذلك استخدم هذا الدليل لنشر **بيئة اختبار خاصة** الآن، ولا توزع التطبيق على الفريق كأداة إنتاجية قبل إصلاح القناع وإعادة اختبار عينات متنوعة.
...
3. افتح إعدادات الخدمة وأدخل الأوامر صراحةً لتفادي أي تخمين من المنصة:
...
## 8. خطة اختبار آمنة قبل الاستخدام الفعلي
